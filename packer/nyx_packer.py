...
def compile(config):
    ...
    download_script += "./hget harness harness\n"
    download_script += "chmod +x harness\n"
    ...
    download_script += "./hget %s target.tar.gz\n"%(os.path.basename(config.argument_values["binary_file"]))
    download_script += "mkdir /tmp/target 2>&1 | ./hcat\n"
    download_script += "tar -xf target.tar.gz -C /tmp/target 2>&1 | ./hcat\n"
    download_script += "rm target.tar.gz 2>&1 | ./hcat\n"
    # download_script += "cp certs/* /tmp/target/\n"
    # download_script += "cp quic_converter_nyx /tmp/target/quic_converter_nyx\n"
    # download_script += "cp start/%s.sh /tmp/target/start.sh\n"%(os.path.basename(config.argument_values["binary_file"]).split(".")[0])
    # download_script += "cp judge/%s-judge.sh /tmp/target/judge.sh\n"%(os.path.basename(config.argument_values["binary_file"]).split(".")[0])
    # download_script += "cd /tmp/target && /tmp/target/setup.sh 2>&1\n"
    # download_script += "if [ $? -ne 0 ]; then\n"
    # download_script += "    /tmp/habort \"Error: setup.sh failed!\"\n"
    # download_script += "fi\n"
    # download_script += "cd -\n"
    ...
    if LEGACY_MODE:
        ...
        copyfile(ld_preload_fuzz_file_legacy, "%s/%s"%(config.argument_values["output_dir"], os.path.basename(ld_preload_fuzz_file)))
        harness_file = agent_folder + "harness"
        copyfile(harness_file, "%s/%s"%(config.argument_values["output_dir"], os.path.basename(harness_file)))
    else:
        ld_preload_fuzz_file = agent_folder + "ld_preload_fuzz.so"
    ...
    # if ld_linux:
    #     download_script += "./%s ./harness %s"%(ld_linux, config.argument_values["args"]) # fixme
    # else:
    download_script += "./harness %s"%(config.argument_values["args"]) # fixme
